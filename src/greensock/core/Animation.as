...
package greensock.core
{
    ...
    //executes on every tick after the core engine updates
		 ...
		 */
		public function Animation(duration : Number = 0,
                                  vars : Object     = null)
		{
			this.vars = vars || {};
		 ...
		 */
		public function play(from : *                 = null,
		 ...
		 * @return self (makes chaining easier)
		 */
		public function pause(atTime : *               = null,
		                      suppressEvents : Boolean = true) : *
		{
			if (atTime != null)
			{
		 ...
		 * @return self (makes chaining easier)
		 */
		public function resume(from : *                 = null,
		                       suppressEvents : Boolean = true) : *
		{
			if (from != null)
			{
		 ...
		 * @return self (makes chaining easier)
		 */
		public function restart(includeDelay : Boolean   = false,
		                        suppressEvents : Boolean = true) : *
		{
			reversed(false);
			paused(false);
			return totalTime((includeDelay ? -_delay : 0),
			                 suppressEvents,
			                 true);
		}
     ...
     myAnimation.reverse();
		 ...
		 * @return self (makes chaining easier)
		 */
		public function reverse(from : *                 = null,
		                        suppressEvents : Boolean = true) : *
		{
			if (from != null)
			{
				seek((from || totalDuration()),
				     suppressEvents);
			}
			reversed(true);
			return paused(false);
		 ...
		 */
		public function render(time : Number,
		 ...
		 **/
		public function _enabled(enabled : Boolean,
		...
		public function _kill(vars : Object   = null,
		 ...
		 * @return self (makes chaining easier)
		 **/
		public function kill(vars : Object   = null,
		                     target : Object = null) : *
		{
			_kill(vars,
			      target);
		...
		{
			_rootFrame++;
			_rootTimeline.render((getTimer() / 1000 - _rootTimeline._startTime) * _rootTimeline._timeScale,
			                     false,
			                     false);
			_rootFramesTimeline.render((_rootFrame - _rootFramesTimeline._startTime) * _rootFramesTimeline._timeScale,
			                           false,
			                           false);
		...
		{
			var i : int = params.length, copy : Array = params.concat();
			while (--i > -1)
			{
				if (params[i] === "{self}")
				{
		 ...
		 */
		public function eventCallback(type : String,
			...
			if (_timeline.smoothChildTiming)
			{
				if (_time > 0)
				{
					if (_time < _duration)
					{
						if (value != 0)
						{
							totalTime(_totalTime * (value / _duration),
							          true);
						}
					}
				}
		 ...
		 * @see #totalTime()
		 **/
		public function time(value : Number           = NaN,
		                     suppressEvents : Boolean = false) : *
		{
			if (!arguments.length)
			{
		 ...
		 * @see #pause()
		 **/
		public function totalTime(time : Number            = NaN,
		                          suppressEvents : Boolean = false,
		...
		public function progress(value : Number           = NaN,
		                         suppressEvents : Boolean = false) : *
		{
			return (!arguments.length) ? _time / duration() : totalTime(duration() * value,
			                                                            suppressEvents);
		}
		...
		public function startTime(value : Number = NaN) : *
		{
			...
			if (value != _startTime)
			{
				...
				if (timeline)
				{
					...
					{
						timeline.add(this,
						             value - _delay); //ensures that any necessary re-sequencing of Animations in the timeline occurs to make sure the rendering order is correct.
					}
				}
			}
			return this;
		...
		public function reversed(value : Boolean = false) : *
		{
			...
			{
				_reversed = value;
				totalTime(((_timeline && !_timeline.smoothChildTiming) ? totalDuration() - _totalTime : _totalTime),
				          true);
			}
			return this;
		}
		...
		public function paused(value : Boolean = false) : *
		{
			...
			if (value != _paused)
			{
				if (_timeline)
				{
					var raw : Number = _timeline.rawTime(), elapsed : Number = raw - _pauseTime;
					...
					{
						render((_timeline.smoothChildTiming ? _totalTime : (raw - _startTime) / _timeScale),
						       true,
						       true); //in case the target's properties changed via some other tween or manual update by the user, we should force a render.
					}
				}
			}
			if (_gc && !value)

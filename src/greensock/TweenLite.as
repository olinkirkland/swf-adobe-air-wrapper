...
package greensock
{
	...
	public class TweenLite extends Animation
	{
		...
		public static var defaultEase : Ease = new Ease(null,
		                                                null,
		                                                1,
		                                                1);
 	...
 	//executes on every tick after the core engine updates
		 ...
		 */
		public function TweenLite(target : Object,
                                  duration : Number,
                                  vars : Object)
		{
			super(duration,
			...
			if (!_overwriteLookup)
			{
				...
				ticker.addEventListener("enterFrame",
				                        _dumpGarbage,
				                        false,
				                        -1,
				                        true);
			...
			if (this.target is Array && typeof (this.target[0]) === "object")
			{
				...
				while (--i > -1)
				{
					_siblings[i] = _register(_targets[i],
					                         this,
					                         false);
					if (_overwrite == 1)
					{
			...
			else
			{
				...
				if (_siblings == null)
				{ //the next few lines accomplish the same thing as _siblings = _register(target, this, false) but faster and only slightly more verbose.
		...
		{
			var immediate : Boolean = vars.immediateRender, i : int, initPlugins : Boolean, pt : PropTween, p : String,
			    copy : Object;
			if (vars.startAt)
			{
				if (_startAt != null)
			...
			{
				_ease = (vars.easeParams is Array) ? vars.ease.config.apply(vars.ease,
				                                                            vars.easeParams) : vars.ease;
			}
			else if (typeof (vars.ease) === "function")
			{
				_ease = new Ease(vars.ease,
				                 vars.easeParams);
			}
			else
			{
			...
			if (_targets)
			{
				...
				while (--i > -1)
				{
					if (_initProps(_targets[i],
					               (_propLookup[i] = {}),
					               _siblings[i],
					               (_overwrittenProps ? _overwrittenProps[i] : null)))
					{
						initPlugins = true;
					}
				}
			...
			if (initPlugins)
			{
				_onPluginEvent("_onInitAllProps",
				               this); //reorders the array in order of priority. Uses a static TweenPlugin method in order to minimize file size in TweenLite
			}
			if (_overwrittenProps)
			{
				if (_firstPT == null)
				{
					...
					{ //if all tweening properties have been overwritten, kill the tween. If the target is a function, it's most likely a delayedCall so let it live.
						_enabled(false,
						         false);
					}
				}
		...
		protected function _initProps(target : Object,
		                              propLookup : Object,
		                              siblings : Array,
		                              overwrittenProps : Object) : Boolean
		{
			var vars : Object = this.vars, p : String, i : int, initPlugins : Boolean, plugin : Object, val : Object;
			if (target == null)
			{
				return false;
			}
			for (p in vars)
			{
				...
				else if ((p in _plugins) && (plugin = new _plugins[p]())._onInitTween(target,
				                                                                      val,
				                                                                      this))
				{
					_firstPT = new PropTween(plugin,
					                         "setRatio",
					                         0,
					                         1,
					                         p,
				...
				{
					_firstPT   = propLookup[p] = new PropTween(target,
					                                           p,
					                                           0,
					                                           1,
					                                           p,
			...
			if (overwrittenProps)
			{
				if (_kill(overwrittenProps,
				          target))
				{ //another tween may have tried to overwrite properties of this tween before init() was called (like if two tweens start at the same time, the one created second will run first)
					return _initProps(target,
					                  propLookup,
					                  siblings,
					                  overwrittenProps);
				}
		...
		override public function render(time : Number,
		                                suppressEvents : Boolean = false,
		                                force : Boolean          = false) : void
		{
			var isComplete : Boolean, callback : String, pt : PropTween, rawPrevTime : Number,
			...
			if (callback)
			{
				...
				{ //check gc because there's a chance that kill() could be called in an onUpdate
					...
					if (!suppressEvents)
					{
						if (vars[callback])
						{
							vars[callback].apply(null,
							                     vars[callback + "Params"]);
						}
					}
		...
		override public function _kill(vars : Object   = null,
		...
		override public function _enabled(enabled : Boolean,
		                                  ignoreTimeline : Boolean = false) : Boolean
		{
			...
			if (_notifyPluginsOfEnabled)
			{
				if (_firstPT != null)
				{
					return _onPluginEvent(((enabled) ? "_onEnable" : "_onDisable"),
					                      this);
				}
			}
			return false;
		 ...
		 * @see #fromTo()
		 */
		public static function to(target : Object,
		                          duration : Number,
		                          vars : Object) : TweenLite
		{
			return new TweenLite(target,
			                     duration,
			                     vars);
		}
		 ...
		 */
		public static function from(target : Object,
		                            duration : Number,
		                            vars : Object) : TweenLite
		{
			vars              = _prepVars(vars,
		 ...
		 */
		public static function fromTo(target : Object,
		                              duration : Number,
		                              fromVars : Object,
		...
		protected static function _prepVars(vars : Object,
		                                    immediateRender : Boolean = false) : Object
		{
			if (vars._isGSVars)
			{
				vars = vars.vars;
    ...
    //do stuff
		 ...
		 */
		public static function delayedCall(delay : Number,
		                                   callback : Function,
		                                   params : Array      = null,
		                                   useFrames : Boolean = false) : TweenLite
		{
			return new TweenLite(callback,
			                     0,
			                     {
				                     delay:                   delay,
				                     onComplete:              callback,
		 ...
		 */
		public static function set(target : Object,
		                           vars : Object) : TweenLite
		{
			return new TweenLite(target,
			                     0,
		 ...
		 **/
		public static function killTweensOf(target : *,
		                                    onlyActive : * = false,
		 ...
		 **/
		public static function getTweensOf(target : *,
		                                   onlyActive : Boolean = false) : Array
		{
		 ...
		 **/
		protected static function _register(target : Object,
		                                    tween : TweenLite = null,
		                                    scrub : Boolean   = false) : Array
		{
			var a : Array = _tweenLookup[target], i : int;
			if (a == null)
			{
		...
		protected static function _applyOverwrite(target : Object,
		                                          tween : TweenLite,
		                                          props : Object,
		                                          mode : int,
		                                          siblings : Array) : Boolean
		{
			...
			while (--i > -1)
			{
				...
				{
					globalStart = globalStart || _checkOverlap(tween,
					                                           0,
					                                           zeroDur);
					if (_checkOverlap(curTween,
					                  globalStart,
					                  zeroDur) === 0)
					{
		 ...
		 **/
		private static function _checkOverlap(tween : Animation,
		                                      reference : Number,
		                                      zeroDur : Boolean) : Number
		{
